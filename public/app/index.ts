...
(function () {
  function createTable(crashes: CrashReport[]) {
    ...
    body.innerHTML = crashes
      .map(({ id, url, tabLastActive, memory }) => {
        return `
                <tr>
                    <td>${id}</td>
                    <td>${url}</td>
